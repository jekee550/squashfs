...
[ -n "$service_name" ] && get_service_data update_url update_script UPD_ANSWER update_binary
[ -z "$update_url" -a -z "$update_script" -a -z "$update_binary" ] && write_log 14 "No update_url found/defined or no update_script found/defined! and no update_binary"
[ -n "$update_script" -a ! -f "$update_script" ] && write_log 14 "Custom update_script not found!"
[ -n "$update_binary" -a ! -f "$update_binary" ] && write_log 14 "Ddns extend binary not found!"
[ -z "$lookup_host" ] && {
...
logger "ddns | regip:$REGISTERED_IP"
