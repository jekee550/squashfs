...
__gwmon_resolve_gw_ip() {
    ...
    if [ -n "$resolved_ip" ]; then
        if [ "$(cat /proc/net/arp | grep -w $resolved_ip | grep $bridge | awk '{print $3}')" == "0x2" ];then
            eval "$2=$resolved_ip"
        fi
    else
...
__gwmon_arping_confirm_gateway() {
    ...
    if [ "$replies" -ge "$gw_reachable_confirm_min_replies" ]; then
        ...
        arping -f -c 1 -w 0 -I "br-lan" -B "$bridge" "$gw_ip" > /dev/null
        __gwmon_info "JD add:update CAP ARP table"
	return 0
    else
...
__gwmon_check_gateway() {
    ...
    for ether_iface in $ether_ifaces; do
        if [ "$ether_iface" = "$__gw_iface" ]; then
            ...
            #add by wangkun271
            [ $(cat /tmp/sysinfo/product_name) = "RE-SS-02"  ] && bksta_ifname="ath25" || bksta_ifname="ath05"
            sta_is_assco=$(iwconfig $bksta_ifname | grep "Access Point:")
            if [ "$sta_iface_5g" = "" -a "$sta_is_assco" != "" ];then
                if [ "$(echo $sta_is_assco | grep "Not-Associated")" = "" ];then
                    __gwmon_info "JD disconnect $bksta_ifname $sta_is_assco"
                    wpa_cli -p "/var/run/wpa_supplicant-$bksta_ifname" -i "$bksta_ifname" disconnect
                fi
            fi
            #end by wangkun271
