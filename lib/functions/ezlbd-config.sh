...
lbd_create_config() {
    ...
    if [ "$multi_ap_mode" = 'map_ctrl' ]; then
        __lbd_cfg_nl_append '[WLANIF]' $filename
        __lbd_cfg_append 'WlanInterfaces=' $filename
        __lbd_cfg_append 'WlanInterfacesExcluded=' $filename
    else
        __lbd_cfg_nl_append '[WLANIF]' $filename
	...
	if [ -n "$multi_ap_mode" ]; then
		...
		if [ "$multi_ap_mode" = 'map' ] || [ "$multi_ap_mode" = 'map_ctrl' ]; then
