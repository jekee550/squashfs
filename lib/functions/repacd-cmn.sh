...
# input：$5  cur_mode: whether to act as a STA or AP
__repacd_init_vap_set_ssid_and_pbc() {
    local name=$1 device=$2 base_ssid=$3 vap_count=$4 cur_mode=$5
    local mode
    local enable_wps_re_enhc global_suffix='' radio_suffix=''
    local start_time duration
    local auto_create_vaps
    local jd_repacd_map_bsta_preference
    local jd_repacd_create_ctrl_bbss
    ...
    config_get jd_repacd_map_bsta_preference "$device" repacd_map_bsta_preference 0
    config_get jd_repacd_create_ctrl_bbss "$device" repacd_create_ctrl_bbss 0
    ...
    if [ $jd_repacd_create_ctrl_bbss -eq 1 -a "$cur_mode" = 'ap' ]; then
        __repacd_update_vap_param "$name" 'channel_block_list' '52,56,60,64,149,153,157,161,165'
    fi
    #Disabling WPS for all additional FH VAPs
    if [ "$vap_count" -eq 0 ]; then
	#只在创建backhual ap或者backhual sta所在的wifi上进行wps_pbc操作
	if [ $jd_repacd_create_ctrl_bbss -eq 1 -o  $jd_repacd_map_bsta_preference -gt 0  -o "$cur_mode" = 'sta' ];then    
...
__repacd_init_vap() {
    ...
    __repacd_init_vap_set_ssid_and_pbc "$name" "$device" "$ssid" "$vap_count" "$mode"
...
__repacd_disable_vap() {
    ...
    config_get wsplcd_unmanaged "$config" wsplcd_unmanaged 0
    config_get repacd_security_unmanaged "$config" repacd_security_unmanaged 0
	...
	if [ ! "$desired_val" = "$disabled" -a "$wsplcd_unmanaged" = "0" -a "$repacd_security_unmanaged" = "0" ]; then
