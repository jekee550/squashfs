...
__repacd_reset_map_default_config() {
    ...
    if [ -z "$fronthaul_ssid" ]; then
        ...
        fronthaul_ssid="JDCwifi-${ssid_suffix}"
    ...
    if [ "$map_version" -gt 1 -a "$map_ts_enabled" -gt 0 -a "$is_controller" -gt 0 ]; then
        ...
        for i in One Two Three; do
            ...
            if [ -z "$fronthaul_ssid" ]; then
                local ssid_suffix
                __repacd_generate_ssid_suffix ssid_suffix
                fronthaul_ssid="JDCwifi-${ssid_suffix}-$i"
                __repacd_generate_psk fronthaul_key
            fi
...
__repacd_create_vaps_map() {
    ...
    local fronthaul_ssid_5G fronthaul_key_5G fronthual_authtype_5G fronthaul_encryption_5G
    local fronthaul_ssid_5GH fronthaul_key_5GH fronthual_authtype_5GH fronthaul_encryption_5GH
    ...
    config_get fronthaul_ssid_5G MAPConfig 'FronthaulSSID5G' ''
    config_get fronthaul_key_5G MAPConfig 'FronthaulKey5G' ''
    config_get fronthual_authtype_5G MAPConfig 'FronthaulAuthType5G' 0
    config_get fronthaul_ssid_5GH MAPConfig 'FronthaulSSID5GH' ''
    config_get fronthaul_key_5GH MAPConfig 'FronthaulKey5GH' ''
    config_get fronthual_authtype_5GH MAPConfig 'FronthaulAuthType5GH' 0
    ...
    if [ -n "$fronthaul_key_5G" ]; then
        fronthaul_encryption_5G='psk2+ccmp'
        [ "$fronthual_authtype_5G" -eq 12 ] && fronthaul_encryption_5G='ccmp'
    else
        fronthaul_encryption_5G='none'
    fi

    if [ -n "$fronthaul_key_5GH" ]; then
        fronthaul_encryption_5GH='psk2+ccmp'
        [ "$fronthual_authtype_5GH" -eq 12 ] && fronthaul_encryption_5GH='ccmp'
    else
        fronthaul_encryption_5GH='none'
    fi
    ...
    if [ -z "$fronthaul_ssid_5G" ]; then
        fronthaul_ssid_5G="${fronthaul_ssid}_5G"
        fronthaul_key_5G="$fronthaul_key"
        fronthaul_authtype_5G="$fronthaul_authtype"
        
        if [ -n "$fronthaul_key_5G" ]; then
            fronthaul_encryption_5G='psk2+ccmp'
            [ "$fronthaul_authtype_5G" -eq 12 ] && fronthaul_encryption_5G='ccmp'
        else
            fronthaul_encryption_5G='none'
        fi
    fi

    if [ -z "$fronthaul_ssid_5GH" ]; then
        fronthaul_ssid_5GH="${fronthaul_ssid}_game"
        fronthaul_key_5GH="$fronthaul_key"
        fronthaul_authtype_5GH="$fronthaul_authtype"
        
        if [ -n "$fronthaul_key_5GH" ]; then
            fronthaul_encryption_5GH='psk2+ccmp'
            [ "$fronthaul_authtype_5GH" -eq 12 ] && fronthaul_encryption_5GH='ccmp'
        else
            fronthaul_encryption_5GH='none'
        fi
    fi
    ...
    for device in $DEVICES; do
        ...
        if [ "$repacd_auto_create_vaps" -gt 0 ]; then
            ...
            if [ "$is_controller" -eq 0 ] || [ "$create_ctrl_fbss" -gt 0 ]; then
                ...
                name=$(uci add wireless wifi-iface)
                uci rename wireless.$name=ath${device:4} #add by wangkun271
                name="ath${device:4}" #add by wangkun271
                config_changed=1

                if whc_is_5g_radio "$device"; then
                        if [ $create_ctrl_bbss -eq 1 ];then
                            __repacd_init_vap "$name" $device 'ap' "$fronthaul_ssid_5G" \
                                      $fronthaul_encryption_5G "$fronthaul_key_5G" '0' "$config_slo_fh_mld"
                        else
                             __repacd_init_vap "$name" $device 'ap' "$fronthaul_ssid_5GH" \
                                      $fronthaul_encryption_5GH "$fronthaul_key_5GH" '0' "$config_slo_fh_mld"
                        fi
                else
                ...
                __repacd_update_vap_param "$name" 'ifname' ath${device:4} #add by wangkun271
		if [ "${device:4}" == "0"  ] ;then
			__repacd_update_vap_param "$name" 'cactimeout' 0
	        fi
                __repacd_update_vap_param "$name" 'map' 1
            ...
            if [ "$is_controller" -gt 0 ] && [ "$create_ctrl_bbss" -gt 0 ] && \
                ...
                name=$(uci add wireless wifi-iface)
                uci rename wireless.$name=ath${device:4}6 #add by wangkun271
                name="ath${device:4}6" #add by wangkun271
                config_changed=1

                __repacd_init_vap "$name" $device 'ap' "$backhaul_ssid" \
                                  $backhaul_encryption "$backhaul_key" '0'
                __repacd_update_vap_param "$name" 'ifname' ath${device:4}6 #add by wangkun271
                __repacd_update_vap_param "$name" 'hidden' 1 #add by wangkun271
		if [ "${device:4}" == "0" ];then
		__repacd_update_vap_param "$name" 'cactimeout' 0 #add by wangkun271
	        fi
                __repacd_update_vap_param "$name" 'map' 1
                __repacd_update_vap_param "$name" 'MapBSSType' 64
...
__repacd_update_map_bsta_radio() {
    ...
    config_get ifnameaddr "$device" macaddr
    ifnameaddr=$(echo "$ifnameaddr" | cut -d ':' -f 4-)
    ifnameaddr="ec:e8:7c:$ifnameaddr"
    ...
    if [ "$3" = "$network" ] && __repacd_is_matching_mode "sta" "$mode"; then
        if [ ! "$device" = "$selected_radio" ]; then
            ...
            uci_set wireless "$config" ifname "ath${selected_radio:4}5" #add by wangkun271 agent vap-bsta ifname=ath15,do not change section name
            uci_set wireless "$config" ifnameaddr "$ifnameaddr" #add by wangkun271  clone mac of athx5
	    changed=$((changed + 1))
            eval "$4='$changed'"
...
__repacd_generate_map_bss_conf() {
    ...
    local fronthaul_ssid_5G  fronthaul_key_5G
    local fronthaul_ssid_5GH  fronthaul_key_5GH
    local backhaul_ssid backhaul_key
    local fronthaul_authtype_str="0x0020" fronthaul_authtype_str_5G="0x0020" fronthaul_authtype_str_5GH="0x0020" backhaul_authtype_str="0x0020"
    local backhaul_r2_authtype_str="0x0020"
    local fronthaul_authtype fronthaul_authtype5G fronthaul_authtype5GH backhaul_authtype
    ...
    config_get fronthaul_authtype5G MAPConfig FronthaulAuthType5G 0
    config_get fronthaul_authtype5GH MAPConfig FronthaulAuthType5GH 0
    ...
    if [ -z "$fronthaul_key_5G" ]; then
        fronthaul_key_5G="NULL"
        fronthaul_authtype_str_5G="0x0001"
    fi

    if [ -z "$fronthaul_key_5GH" ]; then
        fronthaul_key_5GH="NULL"
        fronthaul_authtype_str_5GH="0x0001"
    fi
    ...
    if [ -z "$fronthaul_ssid_5GH" ]; then
        sed -i -e "s!__FH5GH_SSID_REPLACE__!"teardown"!g" "${tempfile}"
        sed -i -e '/Generic-Policy: 1,121,127/s/FH5GH/FH5G/g' "${tempfile}"
    fi

    sed -i -e "s!__FH_SSID_REPLACE__!${fronthaul_ssid/!/\!}!g" \
           -e "s!__FH_KEY_REPLACE__!${fronthaul_key/!/\!}!g" \
           -e "s!__FH5G_SSID_REPLACE__!${fronthaul_ssid_5G/!/\!}!g" \
           -e "s!__FH5G_KEY_REPLACE__!${fronthaul_key_5G/!/\!}!g" \
           -e "s!__FH5GH_SSID_REPLACE__!${fronthaul_ssid_5GH/!/\!}!g" \
           -e "s!__FH5GH_KEY_REPLACE__!${fronthaul_key_5GH/!/\!}!g" \
           ...
           -e "s!__FH5G_AUTH_TYPE_REPLACE__!${fronthaul_authtype_str_5G/!/\!}!g" \
           -e "s!__FH5GH_AUTH_TYPE_REPLACE__!${fronthaul_authtype_str_5GH/!/\!}!g" \
   ...
   while [ $ad_vap_index -le $MAP_MAX_ADDITIONA_VAP ]; do
    ...
    if [ ! -r "${WSPLCD_MAP_BSS_POLICY_PATH}" ] || \
        ...
        # the change count.
        [ ! -f ${WSPLCD_MAP_BSS_POLICY_PATH} ] && mv -f "${tempfile}" "${WSPLCD_MAP_BSS_POLICY_PATH}" #Modify by wangkun271
        changed=$((changed + 1))
        eval "$1='$changed'"
    else
...
__start_map() {
    ...
    #add by wangkun271,porting from zhaoyun sdk,
    #AP mesh
    ...
    if ! __repacd_gw_mode; then
        jdap_mode=$(uci -q get system.@system[0].apmode_init)
        if [ $jdap_mode -eq 0 ];then
        __stop_repacd_run
