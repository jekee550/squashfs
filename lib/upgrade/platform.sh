...
get_key() {
        get_image "$@" | dd skip=5 ibs=32 count=2 2>/dev/null
}
...
check_hlos_type() {
	...
	local ver=`dumpimage -l $1 | grep JDBox | awk -F "\\.r" '{print $3}'`
	local type=$(echo "$key" | grep "$2")
	if [ -n "$ver" ]; then
		if [ $ver -lt 1500 ];then
			echo $ver
...
platform_check_image() {
	...
	local mandatory_nmmc_section_found=0
	...
	image_has_mandatory_section $1 ${mandatory_nor_emmc} && {\
		mandatory_section_found=1
		mandatory_nmmc_section_found=1
	}
	...
	if [ "${board}" = "ap-cp03-c2" -o "${board}" = "ap-cp03-c3" ] && [ $mandatory_nmmc_section_found -eq 0 ]; then
		echo "Error: mandatory section(s) missing from \"$1\". Abort..."
		return 1
	fi
	...
	for sec in $(print_sections $1); do
		...
		hlos*)
			...
			[ -z "$(check_hlos_type /tmp/${sec}.bin "JDBoxes")" -o -z "$(dumpimage -l /tmp/${sec}.bin | grep jdc-ss01)" ] && {
				echo "Error: not JDBox firmware. Abort..."
				return 3
			}

			[ "$board" = "ap-cp03-c4" -a -z "$(dumpimage -l /tmp/${sec}.bin | grep AArch64)" ] && {
				echo "Error: product is not right. Abort..."
				return 4
			}
			[ "$board" = "ap-cp03-c2" -o "$board" = "ap-cp03-c3" ] && [ -z "$(dumpimage -l /tmp/${sec}.bin | grep ARM)" ] && {
				echo "Error: product is not right. Abort..."
				return 4
			}
		esac
...
platform_do_upgrade() {
	...
	case "$board" in
	db149 | ap148 | ap145 | ap148_1xx | db149_1xx | db149_2xx | ap145_1xx | ap160 | ap160_2xx | ap161 | ak01_1xx | ap-dk01.1-c1 | ap-dk01.1-c2 | ap-dk04.1-c1 | ap-dk04.1-c2 | ap-dk04.1-c3 | ap-dk04.1-c4 | ap-dk04.1-c5 | ap-dk04.1-c6 | ap-dk05.1-c1 |  ap-dk06.1-c1 | ap-dk07.1-c1 | ap-dk07.1-c2 | ap-dk07.1-c3 | ap-dk07.1-c4 | ap-hk01-c1 | ap-hk01-c2 | ap-hk01-c3 | ap-hk01-c4 | ap-hk01-c5 | ap-hk01-c6 | ap-hk02 | ap-hk06 | ap-hk07 | ap-hk08 | ap-hk09 | ap-hk10-c1 | ap-hk10-c2 | ap-hk11-c1 | ap-hk12 | ap-hk14 | ap-ac01 | ap-ac02 | ap-ac03 | ap-ac04 | ap-oak02 | ap-oak03 | db-hk01 | db-hk02 | ap-cp01-c1 | ap-cp01-c2 | ap-cp01-c3 | ap-cp01-c4 | ap-cp02-c1 | ap-cp03-c1 | ap-cp03-c2 | ap-cp03-c3 | db-cp01 | db-cp02 | mp-emu | ap-mp02.1 | ap-mp03.1 | ap-mp03.1-c2 | ap-mp03.1-c3 | ap-mp03.3 | ap-mp03.3-c2 | ap-mp03.3-c3 | ap-mp03.4-c1 | ap-mp03.4-c2 | ap-mp03.5-c1 | ap-mp03.5-c2 | ap-mp03.6-c1 | ap-mp03.6-c2 | db-mp02.1 | db-mp03.1 | db-mp03.1-c2 | db-mp03.3 | db-mp03.3-c2 | tb-mp04)
