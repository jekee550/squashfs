...
find_mmc_part() {
	...
	[ "$1" = "storage" ] && [ -e /dev/mmcblk0p27 ] && echo "/dev/mmcblk0p27" && return 0
}
...
default_prerm() {
	local name
	name=$(basename ${1%.*})
	[ -f ${IPKG_INSTROOT}/usr/lib/opkg/info/${name}.prerm-pkg ] || IPKG_INSTROOT=/opt
	[ -f ${IPKG_INSTROOT}/usr/lib/opkg/info/${name}.prerm-pkg ] && . ${IPKG_INSTROOT}/usr/lib/opkg/info/${name}.prerm-pkg
...
default_postinst() {
	...
	ret=0
	pkgname=$(basename ${1%.*})
	[ -f ${IPKG_INSTROOT}/usr/lib/opkg/info/${pkgname}.control ] || IPKG_INSTROOT=/opt
