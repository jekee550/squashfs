...
_disable_qcawificfg80211() {
	...
	#Node 0, zone   Normal    338   1046   1005    672    447     66      9      6      0      0      0
	#nss will crash,when order 4 is 0
	[ $device = "wifi2" -a  $(cat /proc/buddyinfo | grep Normal | awk '{print $9}') -le 2 ] && {
		logger "qcawificfg80211 trigger compact memory!"
...
disable_qcawificfg80211() {
	if [ "$1" = "1" ]; then   #wifi_hw_mode only
		...
		lock /var/run/wifilock
		if [ $(echo "$2" | grep -o "ath"  | wc -l) -eq 2 ];then
			for variable in $2
			do
			if echo "$variable" | grep -q "ath"; then
				_disable_qcawificfg80211 $1 $variable
			fi
			done
		else
			_disable_qcawificfg80211 $@
...
enable_vifs_qcawificfg80211() {
	...
	for vif in $vifs; do
		...
		[ -z "$ifname" ] && config_get ifname "$vif" ifname
		if [ ! -z "$vifs_name" -a "$ifname" != "$vifs_name" ] && { [ -z "$vifs_name2" ] || [ "$ifname" != "$vifs_name2" ]; }; then
			continue
		fi
	...
	for vif in $vifs; do
		...
		local start_hostapd= vif_txpower= nosbeacon= wlanaddr= ifnameaddr=""
		local wlanmode
		...
		case "$mode" in
			sta)
				...
				#JD add sta vap custom bssid [HWaddr of ifconfig]
				config_get ifnameaddr "$vif" ifnameaddr  ""
				;;
			adhoc)
		...
		if [ -z "$recover" ] || [ "$recover" -eq "0" ]; then
		    wlanconfig "$ifname" create wlandev "$phy" wlanmode "$pmode" ${wlanaddr:+wlanaddr "$wlanaddr"}  ${ifnameaddr:+-bssid "$ifnameaddr"}  ${nosbeacon:+nosbeacon} -cfg80211
...
#TODO
		...
		#ifdef JDCLOUD_ADD
		#2.4 GHz VHT 256-QAM Broadcom interoperability support
		config_get wifi_mode "$device" hwmode
		if [ "$wifi_mode" = "11axg" -o "$wifi_mode" = "11ng" ]; then
...
detect_qcawificfg80211() {
	...
	for dev in $(ls -d wifi* 2>&-); do
		...
		#ifdef JDCLOUD_ADD
		#set ssid to JDCwifi_XXXX
		lan_mac=$(cat /sys/class/net/eth1/address)
		wifi_mac=$(cat /sys/class/net/${dev}/address)
		mac0=`expr substr "$lan_mac" 16 2`
		mac0=$(echo $mac0 | tr "[a-z]" "[A-Z]")
		mac1=`expr substr "$lan_mac" 13 2`
...
config wifi-iface	ath$devidx
	...
	option wnm      1
	option mode	ap
	option ssid	JDCwifi_${mac1}${mac0}_5G
	option encryption none
	option ifname  ath$devidx
	option cactimeout 5
	option disabled 0

config wifi-iface	ath$devidx$(($devidx + 1))
	option device	wifi$devidx
	option network	lan
	option hidden   0
	option macfilter deny
	...
	option doth	1
	option mode	ap
	option ssid	JDCwifi_VIP_5G
	option encryption none
	option ifname  ath$devidx$(($devidx + 1))
	option isolate  1
	option cactimeout 5
	option disabled 1

config wifi-iface	ath$devidx$(($devidx + 2))
	option device	wifi$devidx
	option network	wan
	option mode	sta
	option ssid	JDCloudwifi_618_5G
	option extap	1
	option athnewind  1
	option encryption psk2+ccmp
	option key	JDCwifi_pass618...
	option ifname  ath$devidx$(($devidx + 2))
	option disabled 1

EOF
	#5G 9024/9074 setting
	elif [ $devidx  == 2 ];then
		cat <<EOF
config wifi-device  wifi$devidx
	option type	qcawificfg80211
...
config wifi-iface	ath$devidx
	...
	option wnm      1
	option mode	ap
	option ssid	JDCwifi_${mac1}${mac0}_Game
	option encryption none
	option ifname  ath$devidx
	option cactimeout 5
	option disabled 0
...
config wifi-iface	ath$devidx$devidx
	option device	wifi$devidx
	option network	wan
	option mode	sta
	option ssid	JDCloudwifi_618_5G
	...
	option encryption psk2+ccmp
	option key	JDCwifi_pass618...
	option ifname  ath$devidx$devidx
	option disabled 1
...
config wifi-iface	ath$devidx
	...
	option mode	ap
	option ssid	JDCwifi_${mac1}${mac0}
	option encryption none
	option ifname  ath$devidx
	option disabled 0
